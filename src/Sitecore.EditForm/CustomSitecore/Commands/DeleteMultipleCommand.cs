...
{
	public class DeleteMultipleCommand : Command
	{
		/// <summary>
		...
		public override void Execute(CommandContext context)
		{
			if (context.Items.Length == 1)
			{
				...
				Item[] items = selectedItems.Split('|').Select(id => Sitecore.Context.ContentDatabase.GetItem(id)).ToArray();

				//verify items still exist.
				Item[] itemsToDelete = items.Where(x => x.IsNotNull()).ToArray();

				//perform action
				Items.Delete(itemsToDelete);
